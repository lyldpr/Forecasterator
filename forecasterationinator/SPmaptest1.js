...
// shows neighborhoods on startup or returning from properties mode
async function createItems() {
    ...
    // created by sammi and jaxson
    // shift removes the first item in an array (opposite of pop)
    finalTest.shift()
    return finalTest
}

// shows houses after map switches to properties mode
...
async function showHouses(nayborhoodID) {
    ...
    querySnapshot1.forEach((doc) => {
        neighborhoodIDArray.push(doc.data().id)
        item1 = [String(doc.data().address), Number(doc.data().latitude), Number(doc.data().longitude), Number(doc.data().listPrice), Number(doc.data().perNight), count1]
...
// actual initMap function, the main function that creates the map
let locationsList = []
async function initMap(addrID) {
    ...
    activeOutput.innerHTML = " Neighborhoods: " + (locationsList.length).toString();
    //nayborPrice()

    if (neighLoad == false) {
        activeOutput.innerHTML = " Properties: " + (locationsList.length).toString();
        priceProp.innerHTML = ""
    }
    ...
    */
    ...
    // draw function to apply markers and updating values to markers
    function draw(year) {
        rentalPrices = []
        propPrices = []
        for (i = 0; i < locationsList.length; i++) { 
            
            let propertyPrice = locationsList[i][3]
            let propertyPrice2 = locationsList[i][3]
            if (year != null) {
                propertyPrice2 = (((propertyPrice * 0.04) * ((year - 2022)/10)) + propertyPrice)
            }
            console.log(propertyPrice2)
            let rentPrice = locationsList[i][4]
            let rentPrice2 = locationsList[i][4]
            if (year != null) {
                rentPrice2 = (((rentPrice * 0.07) * ((year - 2022)/10)) + rentPrice)
            }

            propPrices.push(propertyPrice2)
            rentalPrices.push(rentPrice2)
            ...
            google.maps.event.addListener(marker, 'click', (function (marker, i) {
                return function() {
                    if (activeNeighbor == false) {
                        ...
                        infowindow.setContent(locationsList[i][0] + `<br />` + `<br />` + `Property Price: ` + parseInt(propertyPrice2) +
                        `<br />` + `<br />` + `Nightly Price: ` + parseInt(rentPrice2) + `<br />` + `<br />` +
    ...
    // changes the property and rent values on the top bar
    function propChange() {
        let sumOfProp = 0
        for (let i = 0; i < propPrices.length; i++) {
            sumOfProp += propPrices[i]
        }
        priceTotal = sumOfProp / (propPrices.length)
        priceProp.innerHTML = "Average Property Price: $" + parseInt(priceTotal);
    }

    function rentChange() {
        let sumOfRent = 0
        for (let i = 0; i < rentalPrices.length; i++) {
            sumOfRent += rentalPrices[i]
        }
        priceNight = sumOfRent / (rentalPrices.length)
        price.innerHTML = "Average Price Per Night: $" + parseInt(priceNight);
    }
    ...
    /*
    function nayborPrice() {
        console.log("called")
        var x = document.getElementById("priceProp");
    ...
    */
    ...
    // also update property and price changes on the top bar
...
// helper functions to add neighborhoods
// created by sammi and jaxson
